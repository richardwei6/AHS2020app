...
public class Old_NewsRecyclerAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    public Old_NewsRecyclerAdapter(FragmentManager fragmentManager, String[] titles){
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if(viewType != 0) {
            View view = LayoutInflater.from(parent.getContext())
                    .inflate(R.layout.old_news_template, parent, false);
            return new Old_NewsRecyclerAdapter.ViewHolder(view);
        }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        if(getItemViewType(position) != 0) {
            if(position == 0)
                ((Old_NewsRecyclerAdapter.ViewHolder)holder).setDetails(fragmentManager, featuredArticles.toArray(new Article_Slim[0]), titles[position], true);
            else
                ((Old_NewsRecyclerAdapter.ViewHolder)holder).setDetails(fragmentManager, articles.get(position-1).toArray(new Article_Slim[0]), (position < titles.length) ? titles[position] : "", false);
        }
